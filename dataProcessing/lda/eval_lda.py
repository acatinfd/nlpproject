...
def eval_lda(threshold):
    ...
    True_lda = len(true_result_lda)

    false_result_lda = {}
    for res in smallTestSet:
        if res not in true_result_lda:
            false_result_lda[res] = 1
    
    Neg_lda = len(false_result_lda)
    print Neg_lda
    
    for res_id in false_result_lda:
    ...
    if Neg_lda > 0:
        precision = float(trueNeg*1.0/(len(smallTestSet) - Neg_lda))
    ...
    # if True_lda > 0:
    #     precision = float(truePos*1.0/True_lda)
    #     print "precision = ", str(precision) 
    # else:
    #     print 'Nothing at all'
